...
import { OWNERS, BOT_TOKEN, WEBHOOK_SECRET } from "./config.js";
...
async function activateClone(botId, botData, origin) {
  ...
  const wh = await cloneTg.setWebhook(`${origin}/tg/clone/${botId}`, WEBHOOK_SECRET);
  if (wh.ok) await kv.saveClone(botId, { ...botData, status: "approved" });
  return wh;
}
...
async function handleTokenSubmission(msg, tg, origin) {
  ...
  if (isOwner(ownerId)) {
    const wh = await checkTg.setWebhook(`${origin}/tg/clone/${botId}`, WEBHOOK_SECRET);
